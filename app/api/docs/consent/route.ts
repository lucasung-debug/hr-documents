...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error(`[docs/consent] Preview generation failed for ${documentKey}`)
    }

    // Update Google Sheets status
    const statusResult = await findDocStatusByEmployeeId(employeeId)
    if (statusResult) {
      await updateDocumentStatus(statusResult.rowIndex, documentKey, 'signed')
    }
    ...
    console.error('[docs/consent] error:', err instanceof Error ? err.message : 'unknown')
    return NextResponse.json({ error: '서류 동의 처리 중 오류가 발생했습니다.' }, { status: 500 })
  }
}
